...
int main(int argc, char *argv[]) {
    ...
    const char *nome_arquivo = (argc > 1) ? argv[1] : "exemplo.dat";

    Grafo *grafo = ler_grafo(nome_arquivo);
    
    srand(time(NULL)); // inicializa semente aleatória uma única vez
    if (!grafo) {
        return EXIT_FAILURE;
    }
    ...
    //Heurística do algoritmo de busca local
    inicio = clock();
    resultado = buscaLocal(grafo->matriz, grafo->num_nos, 0);
    fim = clock();
    tempo_gasto = (double)(fim - inicio) / CLOCKS_PER_SEC;
    printf("Busca Local: %f %f\n", resultado, tempo_gasto);

    //Heurística do algoritmo evolutivo
    inicio = clock();
    resultado = evolutivo(grafo->matriz, grafo->num_nos, 0);
    fim = clock();
    tempo_gasto = (double)(fim - inicio) / CLOCKS_PER_SEC;
    printf("Evolutivo: %f %f\n", resultado, tempo_gasto);
